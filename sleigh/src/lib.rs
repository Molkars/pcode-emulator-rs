...
impl From<&sleigh_sys::ffi::AddrSpace> for AddrSpace {
    fn from(space: &sleigh_sys::ffi::AddrSpace) -> Self {
        ...
        let type_ = SpaceType::from_u32(t).unwrap();
        let name = space.getName().to_string();
        let is_big_endian = space.isBigEndian();
        let wordsize = space.getWordSize();
        Self { name, type_, is_big_endian, wordsize }
    }
}
...
impl From<&sleigh_sys::ffi::VarnodeData> for VarnodeData {
    fn from(var: &sleigh_sys::ffi::VarnodeData) -> Self {
        ...
        let space = sleigh_sys::ffi::getVarnodeSpace(var);
        let space = AddrSpace::from(unsafe {
            space.as_ref().unwrap()
        });
        let offset = sleigh_sys::ffi::getVarnodeOffset(var);
...
impl sleigh_sys::PCodeEmit for PCodeEmit {
    fn dump(
        ...
        let vars = vars.iter().map(|v| VarnodeData::from(*v)).collect::<Vec<_>>();
